...
vector<complex<double>> f_calc_custom_gaussian_parameters(
    py::object structure, vector<vector<int>> hkl, vector<string> atom_labels,
    vector<vector<double>> a, vector<vector<double>> b, vector<double> c) {
    int i;
    ...
    for (i = 0; i < crystal.atoms.size(); i++) {
        ...
        if (!found)
            throw AssertionError(
                "crystal.atoms[i].type in atom_labels", __FILE__, __LINE__);
    }
    ...
    for (i = 0; i < atom_labels.size(); i++) {
        scatterers[atom_labels[i]] =
            NGaussianFormFactor(atom_labels[i], a[i], b[i], c[i]);
    }
