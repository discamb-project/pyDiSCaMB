...
// both with different input parameters (instead of creating them inside the funcitons). 
// File input features are also removed.
...
void printDetailedStats(
    ...
    int nAssignedAtoms,
    int nNotAssignedAtoms,
	int nConsideredStructures,
	int nCompletelyRecognizedStructures)
{
        ofstream out(fName);
        int nTypes, typeIdx, i, nInstances, nSubtypes, subtypeIdx, atomIdx, structureIdx, nNeighbours, neighbourIdx;
    ...
    for (typeIdx = 0; typeIdx < nTypes; typeIdx++)
    {
        ...
        for (subtypeIdx=0 ; subtypeIdx < nSubtypes ; subtypeIdx++)
        {
            ...
            for (i = 0; i < nInstances; i++)
            {
                ...
                for (int j = 0; j < nNeighbours; j++)  // central atom 1-st neighbors
                {
        ...
        for (i = 0; i < nInstances; i++)
        {
            ...
            for (int neighIdx = 0; neighIdx < instanceAtomsNeighbours[i].neighbors.size(); neighIdx++)
            {
                if (neighIdx != 0)
                    formula += ";";
        ...
        sort(neighbourGroups.begin(), neighbourGroups.end(), greater< pair<int, string> >());

        for (auto const &group : neighbourGroups)
        {
            const vector<pair<int, int> > &cases = formulaGrouppedInstances[group.second];
        ...
        for (subtypeIdx = 0; subtypeIdx < nSubtypes; subtypeIdx++)
        {
            ...
            for (i = 0; i < nInstances; i++)
            {
                ...
                for (int j = 0; j < nNeighbours; j++)  // central atom 1-st neighbors
                {
                    neighbourIdx = structuresDescriptors[structureIdx].connectivity[atomIdx][j];
                    instanceAtomsNeighbours[instancesCounter].neighbors[j].symbol = firstNeighborsFormula.first = periodic_table::symbol(structuresDescriptors[structureIdx].atomDescriptors[neighbourIdx].atomicNumber);
                    if (structuresDescriptors[structureIdx].atomDescriptors[neighbourIdx].planar == Tribool::True)
                        instanceAtomsNeighbours[instancesCounter].neighbors[j].symbol += ",planar";
                    if (structuresDescriptors[structureIdx].atomDescriptors[neighbourIdx].planar == Tribool::False)
                    ...
                    int n3Rings = structuresDescriptors[structureIdx].atomDescriptors[neighbourIdx].n3memberRings;
                    int n4Rings = structuresDescriptors[structureIdx].atomDescriptors[neighbourIdx].n4memberRings;
...
typedef tuple<int, string, string, string, int, int, string > UnassignedAtomDescriptors;
    ...
    const vector<int> &nAtomsInAsymmetricUnit)
{
    ...
    for (structureIdx = 0; structureIdx < nStructures; structureIdx++)
    {
        ...
        for (int ringIdx = 0; ringIdx < structureDescriptors[structureIdx].planarRings.size(); ringIdx++)
        {
            out << "\n  ring " << ringIdx + 1 << "\n    planarity esd " << structureDescriptors[structureIdx].planarRingsPlanarityEsd[ringIdx];
            out << "\n    atoms: ";
            for (int i = 0; i < structureDescriptors[structureIdx].planarRings[ringIdx].size(); i++)
        ...
        for (int atomIdx = 0; atomIdx < nAtoms; atomIdx++)
        {
            ...
            string symbol;
            vector<int> _v {
                structureDescriptors[structureIdx].atomDescriptors[atomIdx].neighborsFormula.begin(), 
                structureDescriptors[structureIdx].atomDescriptors[atomIdx].neighborsFormula.end()
                };
            string formulaStr = atomicNumbers2Formula(_v);
            string planarityStr;

            planarityStr = atom_typing_utilities::planarity_as_string(structureDescriptors[structureIdx].atomDescriptors[atomIdx].planar);
            ...
            out << endl;
        } // for(int atomIdx = 0; atomIdx < structreDescriptors[moleculeIdx].atomDescriptors.size(); atomIdx++)
	...
	map< UnassignedAtomDescriptors, vector<string> > &sortedUnassignedAtoms)
{
	...
	sort(groupMultiplicityAndIndex.begin(), groupMultiplicityAndIndex.end(), greater< pair<int, int> >());
...
void write_assignment_logs(
    ...
    const StructureWithDescriptors structure,
    ...
    try {
        ...
        nCompletelyRecognizedStructures = (find(typeIds.begin(), typeIds.end(), -1) == typeIds.end());
        ...
        vector< vector<vector<pair<int, int> > > > typeInstances;
        vector<pair<string, vector<int> > > multitypes; //[i].first - label [i].second - list of indices of types

        findMultitypes(atomTypes, multitypes);
        findTypeInstances(multitypes, typeIndices, typeInstances, atomTypes.size());

        printDetailedStats(
            "type_instances_info.txt", 
            "", 
            multitypes, 
            typeInstances, 
            structureIds, 
        ...
        printMoreDetailedDetailedStats(
            "type_instances_detailed_info.txt", 
            "", 
            multitypes, 
            typeInstances, 
            structureIds, 
        ...
        printUnassignedAtomsInfo("unassigned_atoms_info.txt", "", sortedUnassignedAtoms);        
		printUnassignedAtomsInfoSortedByN("unassigned_atoms_sorted_by_n.txt", "", sortedUnassignedAtoms);
