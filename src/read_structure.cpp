...
void update_crystal_from_xray_structure(discamb::Crystal &crystal, const py::object structure){
    ...
    for (auto scatterer_py : structure.attr("scatterers")()){
        ...
        if (crystal.atoms[idx].label.size()){
            assert(crystal.atoms[idx].label == scatterer_py.attr("label").cast<string>());
            assert(crystal.atoms[idx].type == scatterer_py.attr("scattering_type").cast<string>());
        }
        else {
            crystal.atoms[idx].type = scatterer_py.attr("scattering_type").cast<string>();
            crystal.atoms[idx].label = scatterer_py.attr("label").cast<string>();
        }
        ...
        if (scatterer_py.attr("flags").attr("use_u_iso")().cast<bool>()){
            ...
            // TODO check if cctbx has the same order. Comments below are discamb order
...
void update_anomalous_from_xray_structure(vector<complex<double>> &anomalous, const py::object structure){
